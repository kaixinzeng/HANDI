...
def main():
    ...
    filename_mse = [
        args.path_edmd.format(dt=f"{DT:.1f}"),
        args.path_sindy.format(dt=f"{DT:.1f}"), 
        args.path_handi.format(dt=f"{DT:.1f}"),
        args.path_pse.format(dt=f"{DT:.1f}"),
        args.path_gedmd.format(dt=f"{DT:.1f}"),
        args.path_wsindy.format(dt=f"{DT:.1f}"),
        args.path_sr3.format(dt=f"{DT:.1f}")
    ]
...
def parse_arguments():
    ...
    parser.add_argument("--true_data_file",type=str, default="pendulum/pendulum_dt0.1.npy")
    parser.add_argument("--system", type=str, default="pendulum", help="System name for output csv, e.g., NI")
    parser.add_argument("--dt", type=float,default=0.1, help="Time step of the trained data")
    parser.add_argument("--original_dt", type=float, default=1/120, help="Time step of the original data")
    parser.add_argument("--path_edmd", type=str, default="pendulum/results/EDMD/dt{dt}_equations.txt")
    parser.add_argument("--path_sindy", type=str,default="pendulum/results/SINDy/dt{dt}_equations.txt")
    parser.add_argument("--path_handi", type=str,default="pendulum/results/HANDI/dt{dt}_equations.txt")
    parser.add_argument("--path_pse", type=str,default="pendulum/results/PSE/dt{dt}_equations.txt")
    parser.add_argument("--path_gedmd", type=str,default="pendulum/results/gEDMD/dt{dt}_equations.txt")
    parser.add_argument("--path_wsindy", type=str,default="pendulum/results/WSINDy/dt{dt}_equations.txt")
    parser.add_argument("--path_sr3", type=str,default="pendulum/results/SR3/dt{dt}_equations.txt")
    parser.add_argument("--mse_steps", nargs='*',default=[None], help="Steps for MSE slicing, None means full length")
    parser.add_argument("--output_dir", type=str,default="nrmse_mse_r2/pendulum/results/test")
