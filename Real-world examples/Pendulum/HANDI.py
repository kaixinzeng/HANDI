...
def main():
    ...
    # data & dictionary
    p.add_argument("--data", type=str, default="pendulum_sindy_data.npy", help="path to npy array of shape (N,T,d)")
    p.add_argument("--dt", type=float, default=0.3)
    p.add_argument("--polyorder", type=int, default=2)
    # data sampling
    p.add_argument("--data_dt", type=float, default=0.0083333333,
                   help="Original data sampling time (seconds), default 1/120 ≈ 0.0083333")
    p.add_argument("--stride", type=int, default=36,
                   help="Manual downsampling stride: take one sample every `stride` steps (1 means no downsampling)")

    # model
    p.add_argument("--g_dim", type=int, default=4)
    ...
    p.add_argument("--outdir", type=str, default="./pendulum_runs003V5")
    p.add_argument("--true_coeff_json", type=str,
                   default="[[1.1, 0.0],[0.0, -0.4],[0.0, 0.0],[-0.4, 0.1],[0.0, 0.0]]",
                   help="Ground truth monomial coefficient matrix as a JSON string; leave empty to skip NRMSE calculation")
    ...
    p.add_argument("--hp_json", type=str, default="", help="JSON dict to override hyperparameters for this run (for compatibility)")
    p.add_argument("--top_r", type=int, default=4,
                   help="Total number of coefficients to keep for IVP evaluation (summed across all states), <=0 means no pruning")
