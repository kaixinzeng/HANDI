...
def main():
    ...
    filename_mse = [
        args.path_edmd.format(dt=f"{DT:.4f}"),
        args.path_sindy.format(dt=f"{DT:.2f}"), 
        args.path_handi.format(dt=f"{DT:.3f}"),
        args.path_pse.format(dt=f"{DT:.2f}"),
        args.path_gedmd.format(dt=f"{DT:.2f}"),
        args.path_wsindy.format(dt=f"{DT:.3f}"),
        args.path_sr3.format(dt=f"{DT:.3f}")
    ]
    ...
    for n in tqdm(range(num_traj)):
        ...
        for i,f in enumerate(filename_mse):
            ...
            sol = solve_ivp(lambda t, x: model(x, t),[t_eval_original[0], t_eval_original[-1]],x0,t_eval=t_eval_original,method='RK45',rtol=1e-6,atol=1e-8)
...
def parse_arguments():
    ...
    parser.add_argument("--true_data_file",type=str, default="mse_eletric/pe/NWB_traj2_states=3_t=1.6_zscore_num60.npy")
    parser.add_argument("--system", type=str, default="NI", help="System name for output csv, e.g., NI")
    parser.add_argument("--dt", type=float,default=0.01, help="Time step of the trained data")
    parser.add_argument("--original_dt", type=float, default=1/200000, help="Time step of the original data")
    parser.add_argument("--path_edmd", type=str, default="mse_eletric/pe/results/EDMD/dt{dt}/EDMD_equation_poly_10_sysid_3_dt_{dt}.txt")
    parser.add_argument("--path_sindy", type=str,default="mse_eletric/pe/results/sindy/sindy_equations_dt{dt}.txt")
    parser.add_argument("--path_handi", type=str,default="mse_eletric/pe/results/HANDI/dt{dt}/best_equations_dt{dt}.txt")
    parser.add_argument("--path_pse", type=str,default="mse_eletric/pe/results/PSE/dt{dt}/pse_equation_dt{dt}.txt")
    parser.add_argument("--path_gedmd", type=str,default="mse_eletric/pe/results/gedmd/gedmd_dt{dt}/best_equations.txt")
    parser.add_argument("--path_wsindy", type=str,default="mse_eletric/pe/results/wsindy/dt{dt}/tune_best_equations.txt")
    parser.add_argument("--path_sr3", type=str,default="mse_eletric/pe/results/sr3/NI_dt{dt}/sindy_sr3_equations.txt")
    parser.add_argument("--mse_steps", nargs='*',default=[None], help="Steps for MSE slicing, None means full length")
    parser.add_argument("--output_dir", type=str,default="nrmse_mse_r2/NI_new/results/test")
