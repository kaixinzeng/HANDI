...
def build_poly(X, I, polyorder=3):
    N, d = X.shape
    basis, names = [], []
    for i in range(d):
        basis.append(X[:, i:i+1]); names.append(f'x({i+1})')
    basis.append(I); names.append(f'x({d+1})')
    # for i, b in enumerate(basis):
    #     print(f"[{i}] shape={np.shape(b)}, name={names[i]}")
...
def compute_nrmse(C_hat: np.ndarray, C_true: np.ndarray) -> float:
    ...
    n_basis_hat = C_hat.shape[0]
    n_basis_true = C_true.shape[0]
    d = C_true.shape[1] 
    N_max = max(n_basis_hat, n_basis_true)
    if n_basis_hat < N_max:
        C_hat_padded = np.zeros((N_max, d))
        C_hat_padded[:n_basis_hat, :] = C_hat
        C_hat = C_hat_padded
    if n_basis_true < N_max:
        C_true_padded = np.zeros((N_max, d))
        C_true_padded[:n_basis_true, :] = C_true
        C_true = C_true_padded
...
def make_dataloaders(X_all_tensor, polyorder, R_mono2Q_tensor, win_S=5, batch_size=512,
    ...
    Phi_all_mono, _ = build_poly(X_flat, I_flat, polyorder=polyorder)
    ...
    I0 = I[:, :-1].reshape(-1, 1)
    I1 = I[:, 1:].reshape(-1,1)
    Phi_x_mono, _ = build_poly(X0, I0, polyorder=polyorder)
    Phi_y_mono, _ = build_poly(X1, I1, polyorder=polyorder)
    Qx = Phi_x_mono @ Rinv
    Qy = Phi_y_mono @ Rinv
    ...
    I_seq = torch.from_numpy(np.stack(I_seq_list).astype(np.float32))

    ds = TensorDataset(X_seq, Q_seq, RQ_first, I_seq)  
    if val_split > 0.0:
...
class LightningResidualKoopman(pl.LightningModule):
    def __init__(self,
        ...
        I0 = I[:, :-1].reshape(-1, 1)
        I1 = I[:,  1:].reshape(-1, 1)
        Phi_x_mono, names = build_poly(X0, I0, polyorder=polyorder)  # Phi_x_mono, names = build_poly(X0,I, polyorder=polyorder)
        Phi_y_mono, _    = build_poly(X1, I1, polyorder=polyorder)
        ...
        self.register_buffer("X_all_cpu", torch.from_numpy(Xall_np.astype(np.float32)), persistent=False)
    ...
    def training_step(self, batch, batch_idx):
        ...
        X_seq, Q_seq, RQ_first, I_seq = batch
...
def extract_L_C_eqs(module: LightningResidualKoopman):
    ...
    # C_hat = L[:Dp, :module.d] :(module.d+1)
    C_hat = L[:Dp, :(module.d+1)]
...
def evaluate_equation_rollout_mse_ivp(module, val_loader, fine_h: float = 1e-3,
    ...
    def build_phi_torch(x):
        ...
        feats = [x]
        high = []
        for j in range(d+1, len(mono_idx)):  # d
            idxs = mono_idx[j]
    ...
    def f_cont(x):
        Phi = build_phi_torch(x)
        return Phi @ C

    def rk4_step(x, hstep):  # x 3d
        k1 = f_cont(x)
    ...
    for batch in val_loader:
        X_seq, _, _ , I_seq = batch
        X_seq = X_seq.to(device=device, dtype=torch.float64, non_blocking=False)
        I_seq = I_seq.to(device=device, dtype=torch.float64, non_blocking=False)
        B, S, _ = X_seq.shape
        steps = S-1
        
        y_seq = torch.cat([X_seq, I_seq], dim=2)
        x_pred = X_seq[:, 0, :]  #(5, 2)
        I_pred = I_seq[:, 0]
        y_pred = torch.cat([x_pred, I_pred], dim=1)
...
def evaluate_koopman_rollout_mse_disc(module, val_loader, eig_eps: float = 1e-8, verbose: bool = False):
    ...
    for X_seq, Q_seq, _ , _ in val_loader:
        ...
        if S <= 1:
            continue  
        X0 = X_seq[:, 0, :]                                  # [B,d]
        Phi0 = build_phi_torch(X0)                           # [B,Dp]
        dev_model = module.device_
        G_raw0 = module.model(X0.to(dev_model).float()).to(device=device, dtype=torch.float64)  # [B,g]
        Qt = Q_seq[:, 0, :]                                  # [B,Dp]
        XtX = Qt.T @ Qt
        XtY = Qt.T @ G_raw0
        try:
            C = torch.linalg.solve(XtX + eps*I_Dp, XtY)      # [Dp,g]
        except RuntimeError:
            C = torch.linalg.lstsq(XtX + eps*I_Dp, XtY, rcond=None).solution
        G_perp0 = G_raw0 - Qt @ C                            # [B,g]
        Zk = torch.cat([Phi0, G_perp0], dim=1).to(torch.float64)  # [B, Dp+g]

        mse_b = 0.0
        for k in range(1, S):
            Zk = Zk @ Kmono_t
            x_pred = Zk[:, :d].to(torch.float32)             # [B,d]
...
def export_artifacts(module, args, val_loader=None, outdir=None):
    ...
    with open(os.path.join(outdir, f"best_equations_dt{args.dt:.3f}.txt"), "w") as f:
        for line in eqs:
            f.write(line + "\n")
    np.save(os.path.join(outdir, f"best_L_hat_dt{args.dt:.3f}.npy"), L)
...
def main():
    ...
    # data & dictionary
    p.add_argument("--data", type=str, default="NWB_traj2_states=3_t=1.6_zscore_num60.npy", help="path to npy array of shape (N,T,d)")
    p.add_argument("--dt", type=float, default=0.01)
    p.add_argument("--polyorder", type=int, default=3)

    # model
    p.add_argument("--g_dim", type=int, default=56)
    ...
    p.add_argument("--epochs_B", type=int, default=240)
    ...
    p.add_argument("--lam_g", type=float, default=7.683328671301727e-07)
    p.add_argument("--lam_full", type=float, default=7.843878507638142e-07)
    p.add_argument("--rollout_B", type=int, default=16)
    p.add_argument("--beta_align", type=float, default=0.0009257334796905804)
    ...
    p.add_argument("--outdir", type=str, default="./traj2_states=3_t=1.6_zscore_num60")
    p.add_argument("--true_coeff_json", type=str,
                   default=None,
                   help="Ground truth monomial coefficient matrix JSON string; leave empty to skip NRMSE calculation")

    # lightning runtime
    p.add_argument("--devices", type=int, default=1, help="Number of GPUs; >1 enables DDP (must be =4 for trial/final training)")
    p.add_argument("--precision", type=str, default="32-true", help="Precision: 32-true, 16-mixed, bf16-mixed")
    ...
    p.add_argument("--n_trials", type=int, default=9999)
    ...
    p.add_argument("--hp_json", type=str, default="", help="JSON dict to override hyperparameters for this training (for compatibility)")
    p.add_argument("--top_r", type=int, default=4,
               help="Total number of coefficients to retain when evaluating equation IVP (summed across all states); <=0 means no pruning")
