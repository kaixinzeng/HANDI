...
class LightningResidualKoopman(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if inA:
            ...
            proj = Q0 @ C
            G_perp = G_raw - alpha * proj ### G_perp=G_raw 
            pred = G_perp @ self.Kg

            loss_main = torch.mean((pred - RQ_first) ** 2)
            loss_align = self.beta_align * torch.mean((self.Kg - self.Kg_closed) ** 2)  ##可修改
            ...
            K = self.K_aug_Q
            pred1 = Z0 @ K
            loss1 = torch.mean((pred1 - Z_seq[:, 1, :]) ** 2)   ##可修改
...
def export_artifacts(module, args, val_loader=None, outdir=None):
    ...
    with open(os.path.join(outdir, f"cyc_HANDI_best_equations_dt{args.dt:.3f}.txt"), "w") as f:
        for line in eqs:
            f.write(line + "\n")
...
def compute_Phi(args):
    Xall = np.load(args.data)
    checkpoint = torch.load(args.model_weights_path, map_location="cuda")
    # test_data=np.load(args.test_data)
    # test_data=data_generate(args,args.low_threshold,args.high_threshold)
    ...
    with open(os.path.join(args.out_dir_final, f"cyc_HANDI_best_equations_dt{args.dt:.3f}.txt"), "w") as f:
        for line in eqs:
            f.write(line + "\n")
...
def run_tuning(args):
    ...
    def objective(trial: "optuna.trial.Trial"):
        ...
        cfg.width     = trial.suggest_int("width", max(32, base.width-64), base.width+64, step=32)
...
def main():
    ...
    # data & dictionary
    p.add_argument("--dt", type=float, default=0.1, required=True)
    p.add_argument("--rollout_B", type=int, default=30)
    p.add_argument("--num_point", type=int, default=100)
    p.add_argument("--top_k", type=int, default=10)
    p.add_argument("--low_threshold", type=float, default=-4) #低阈值
    p.add_argument("--high_threshold", type=float, default=4) #高阈值
    p.add_argument("--out_dir_final", type=str, default="../results", help="path to save final files")
    
    p.add_argument("--model_weights_path", type=str, default=None, help="path to model weights (auto-generated if not provided based on dt)")
    p.add_argument("--config", type=str, default=None, help="path to config file (auto-generated if not provided based on dt)")
    p.add_argument("--L", type=str, default=None, help="path to L_hat file (auto-generated if not provided based on dt)")
    p.add_argument("--data", type=str, default=None, help="path to npy array of shape (N,T,d)")
