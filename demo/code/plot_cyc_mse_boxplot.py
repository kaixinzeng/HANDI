...
def load_data_multi_dt(root_dir: str) -> Dict[str, Dict[float, Dict[str, pd.Series]]]:
    ...
    file_pattern = re.compile(r'(.+)_mse_dt(\d+\.?\d*).csv$', re.IGNORECASE)
...
def plot_mse_boxplot_per_dt(data: Dict[str, Dict[float, Dict[str, pd.Series]]], output_dir: str):
    ...
    output_mse_dir = os.path.join(output_dir)
    ...
    for system_name in system_names:
        ...
        for dt_value in sorted_dts:
            ...
            plot_fname = f'figure2_{safe_system_name}_dt{dt_value:.2f}_MSE_Boxplot'
            plot_path_svg = os.path.join(output_mse_dir, f'{plot_fname}.pdf')
...
def main():
    ...
    parser.add_argument("--root", type=str, default="../data", help="Root directory to search for MSE result .csv files (e.g., system_mse_all_dt0.1_xyrange1.0_grid10.csv).")
    parser.add_argument("--output", type=str, default="../results", help="Directory to save the generated boxplots.")
    ...
    print(f"Generating Boxplots and saving to: {os.path.join(output_dir)}")
