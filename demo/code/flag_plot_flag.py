...
def main():
    ...
    ap.add_argument("--tmax", type=float, default=8, help="truncate to t<=tmax (in same unit as dt)")
    ap.add_argument("--dim", type=int, default=4)
    # equation files
    ap.add_argument("--truth-npy", type=str, default=None, help="(N,T,4) truth npy")
    ap.add_argument("--handi-eq", type=str, default=None)
    ap.add_argument("--pse-eq", type=str, default=None)
    ap.add_argument("--sindy-eq", type=str, default=None)
    ap.add_argument("--sr3-eq", type=str, default=None)
    ...
    ap.add_argument("--save-format", type=str, default="pdf")
    ...
    args = ap.parse_args()
    dt_str = f"{args.dt:g}"
    args.truth_npy = f"../data/flag_traj_4_dt{dt_str}.npy"
    args.handi_eq = f"../results/HANDI_flag_dt{dt_str}.txt"
    args.pse_eq = f"../data/PSE_flag_dt{dt_str}.txt"  # You mentioned PSE is in data, updating naming convention
    args.sindy_eq = f"../data/SINDy_flag_dt{dt_str}.txt"
    args.sr3_eq = f"../data/SR3_flag_dt{dt_str}.txt"
    ...
    for i in range(plot_dims):
        ...
        ax.set_ylabel(f"x{i+1}", fontsize=STYLE.tick_fontsize)
        
        apply_ticks_adaptive(ax, xspan=xspan)
        apply_spines(ax)
        ax.grid(False)

    axes[-1].set_xlabel("Time (s)", fontsize=STYLE.tick_fontsize)

    if not args.no_legend:
        maybe_add_legend(axes[0])

    out_fig = os.path.join(args.out_dir, f"figure5_flag_d_Ts={args.dt}s.{args.save_format}")
    plt.savefig(out_fig, bbox_inches="tight", pad_inches=0.03)
    plt.close(fig)
