...
def downsample_by_stride(X: np.ndarray, stride: int) -> np.ndarray:
    """
    Downsample a 3D array by taking every `stride`-th sample along the time axis.
    Args:
        X: (N, T, d) numpy array.
        stride: Integer stride (>=1).
    Returns:
        (N, T', d) downsampled numpy array.
    """
    s = max(1, int(stride))
    if s == 1:
        return X
...
class LightningResidualKoopman(pl.LightningModule):
    ...
    def compute_Kg_full_closed_dist(self, alpha=1.0):
        ...
        Qx_local = self.Qx_cpu[s:e].to(dev, non_blocking=True)
        RQ_local = self.RQ_cpu[s:e].to(dev, non_blocking=True)
        Gx_local = self.fwd_all(self.X0_cpu[s:e])

        GT_local = Qx_local.T @ Gx_local
        if is_dist:
            dist.all_reduce(GT_local, op=dist.ReduceOp.SUM)
        GT_global = GT_local

        Gx_perp_local = Gx_local - alpha * (Qx_local @ GT_global)
        GTG_local = Gx_perp_local.T @ Gx_perp_local
        GTR_local = Gx_perp_local.T @ RQ_local
        if is_dist:
...
def extract_L_C_eqs(module: LightningResidualKoopman):
    ...
    C_hat = L[:Dp, :module.d]
    eqs = format_equations(L, module.names, topk=None, thresh=1e-10)
    return L, C_hat, eqs, K_aug_mono
...
def evaluate_equation_rollout_mse_ivp(module, val_loader, fine_h: float = 1e-3,
    ...
    C_np = L[:Dp, :module.d]
    C = torch.from_numpy(C_np.astype(np.float64)).to(device)
...
def export_artifacts(module, args, val_loader=None, outdir=None):
    """Save equations to a single HANDI_InvertedFlag_dt{dt}.txt file."""
    save_dir = RESULT_DIR
    os.makedirs(save_dir, exist_ok=True)

    L, C_hat, eqs, K_mono = extract_L_C_eqs(module)

    # ---- Save single output file ----
    out_file = os.path.join(save_dir, f"HANDI_flag_dt{args.dt}.txt")
...
def fit_once(args, Xall):
    ...
    logger = CSVLogger(save_dir=RESULT_DIR, name="HANDI_logs")

    # trainer setup
    use_ddp = False
    if isinstance(args.devices, int) and args.devices > 1:
        use_ddp = True
    elif isinstance(args.devices, str) and args.devices.isdigit() and int(args.devices) > 1:
        use_ddp = True

    callbacks_list = [SeedInChildren(args.seed)] # Assuming this was the original callback

    trainer = pl.Trainer( # Changed L.Trainer to pl.Trainer to match existing code style
...
def load_and_export(args):
    """Load pre-trained .pt weights from the data directory and generate equations
    without training, following the pattern from handiV1.PY's compute_Phi()."""
    seed_everything_cpu_only(args.seed)

    # ---- Determine paths for .pt and .json in data/ ----
    dt_str = str(args.dt)
    if abs(args.dt - 0.1666) < 1e-4:
        dt_str = "0.1666"

    # Find config JSON
    config_json = os.path.join(DATA_DIR, f"HANDI_dt{dt_str}_config.json")
    if not os.path.exists(config_json):
        print(f"[WARN] Config JSON not found: {config_json}. Using defaults.")
        config_json = None

    # Find .pt model weights
    model_pt = os.path.join(DATA_DIR, f"HANDI_dt{dt_str}_model.pt")
    if not os.path.exists(model_pt):
        raise FileNotFoundError(f"HANDI pre-trained model NOT found: {model_pt}")

    # ---- Load config and apply hyperparameters ----
    if config_json is not None:
        with open(config_json, 'r') as f:
            config = json.load(f)
        print(f"[cfg] loaded HANDI config from {config_json}")

        best_cfg = config.get("best_cfg", config)
        for key in ['stride', 'g_dim', 'width', 'depth', 'dropout', 'epochs_A', 'epochs_B',
                    'lrA', 'lrB_start', 'lrB_end', 'decayA_last_frac', 'decayA_min_ratio',
                    'lam_phi', 'lam_g', 'lam_full', 'rollout_B', 'beta_align',
                    'batch_size', 'grad_clip', 'polyorder', 'seed']:
            if key in best_cfg:
                
                val = best_cfg[key]
                # Force conversion to integer for architecture-critical keys which might have been saved as float occasionally
                if key in ['width', 'depth', 'g_dim', 'stride', 'batch_size', 'epochs_A', 'epochs_B']:
                    try: val = int(val)
                    except: pass
    ...
    # ---- Load data ----
    data_path = args.data
    Xall_raw = np.load(data_path)
    # The data passed via args.data (e.g., flag_traj_4_dt0.1.npy) is already downsampled correctly.
    # Force stride to 1 to prevent double-downsampling the test data.
    Xall = downsample_by_stride(Xall_raw, 1)

    if Xall.ndim != 3:
        raise ValueError(f"Data must be 3D (N,T,d), got shape {Xall.shape}")
    N, T, d = Xall.shape
    ...
    model.eval()
    print(f"[model] loaded weights from {model_pt}")

    # ---- Compute K_aug_Q and extract equations ----
    model.K_aug_Q = model.compute_Kaug_full()
    L, C_hat, eqs, K_mono = extract_L_C_eqs(model)
...
def train_and_export(args):
    ...
    # Load original data
    Xall_raw = np.load(args.data)
    if Xall_raw.ndim != 3:
        raise ValueError("Data file must be a 3D array of shape (N, T, d).")
    N, T_raw, d = Xall_raw.shape
    print(f"[DATA] Original: N={N}, T={T_raw}, d={d}, data_dt={args.data_dt:.9f}s")

    # Manual downsampling
    Xall = downsample_by_stride(Xall_raw, args.stride)
    N2, T_ds, d2 = Xall.shape
    dt_eff = args.data_dt * max(1, int(args.stride))
    print(f"[DATA] Downsampled: N={N2}, T={T_ds}, d={d2}, stride={args.stride}, dt_eff={dt_eff:.9f}s")

    # Consistency check (non-blocking)
    if abs(args.dt - dt_eff) > max(1e-9, 0.01*args.dt):
        print(f"[WARN] The --dt={args.dt:.9f}s used for training does not match the effective data step dt_eff={dt_eff:.9f}s. "
              f"(This is just a warning; training will continue. Adjust --stride or --dt for strict consistency.)")
...
def run_tuning(args):
    ...
    Xall_base = downsample_by_stride(Xall_raw, args.stride)
    N2, T_ds, d2 = Xall_base.shape
    dt_eff = args.data_dt * max(1, int(args.stride))
    print(f"[DATA] Downsampled (for tuning/all trials): N={N2}, T={T_ds}, d={d2}, stride={args.stride}, dt_eff={dt_eff:.9f}s")
    ...
    def objective(trial: "optuna.trial.Trial"):
        ...
        try:
            trainer, module, val_loader = fit_once(cfg, Xall_base)
            ...
            if metric < best_metric:
                ...
                _cp(f"best_L_hat_dt{base.dt:.3f}.npy")

                print(f"[best] Metric improved to {best_metric:.6g}; best artifacts updated.")
    ...
    if len(study.trials) == 0 or study.best_trial is None:
        print("[optuna] No trials completed, falling back to base training.")
...
def main():
    ...
    # data & dictionary
    p.add_argument("--data", type=str, default="", help="path to npy array of shape (N,T,d)")
    p.add_argument("--dt", type=float, default=0.2)
    p.add_argument("--polyorder", type=int, default=3)
    # data sampling
    p.add_argument("--data_dt", type=float, default=0.0333,
                   help="Original data sampling time (seconds)")
    p.add_argument("--stride", type=int, default=6,
                   help="Manual downsampling stride (1 means no downsampling)")

    # model
    p.add_argument("--g_dim", type=int, default=5)
    p.add_argument("--width", type=int, default=256)
    p.add_argument("--depth", type=int, default=2)
    p.add_argument("--dropout", type=float, default=0.25)

    # training
    p.add_argument("--epochs_A", type=int, default=200)
    ...
    p.add_argument("--rollout_B", type=int, default=10)
    ...
    p.add_argument("--outdir", type=str, default=RESULT_DIR)
    p.add_argument("--true_coeff_json", type=str,
                   default="[[1.1, 0.0],[0.0, -0.4],[0.0, 0.0],[-0.4, 0.1],[0.0, 0.0]]",
                   help="Ground truth coefficient matrix as a JSON string; leave empty to skip NRMSE calculation")

    # lightning runtime
    p.add_argument("--devices", type=int, default=1, help="GPUs to use (0=CPU, 1=1GPU, etc. or 'auto')")
    p.add_argument("--precision", type=str, default="32-true", help="Precision: 32-true/16-mixed/bf16-mixed etc.")
    p.add_argument("--max_epochs", type=int, default=-1, help="Override epochs_A+epochs_B; -1 uses A+B")
    ...
    p.add_argument("--tune_save_trials", type=int, default=0, help="Save each trial's artifacts (1=yes, 0=no)")
    p.add_argument("--hp_json", type=str, default="", help="JSON dict to override hyperparameters (for compatibility)")
    p.add_argument("--top_r", type=int, default=4,
                   help="Total number of coefficients to keep for IVP evaluation (<=0 means no pruning)")
    ...
    if args.hp_json:
        try:
            ...
            print(f"[hp_json WARN] Failed to parse: {e}")

    if not args.data:
        args.data = os.path.join(DATA_DIR, f"flag_traj_4_dt{args.dt}.npy")
    ...
    if args.tune:
        ...
        # ---- Load pre-trained .pt model and generate equations ----
        load_and_export(args)
