...
eig_values = np.load(f'../data/cyc_eigenfunction_{dt}_eigvals.npy')
data_points = np.load(f'../data/cyc_eigenfunction_{dt}.npy')
Y = np.load(f'../data/cyc_eigenfunction_{dt}_Y.npy')
X = np.load(f'../data/cyc_eigenfunction_{dt}_X.npy')
...
for i in range(11,12): # vdp 13, cyc 11, duffing 0, fix 1
    ...
    plt.tight_layout()
    plt.savefig(f'../results/figure2_{system}_eigenfunction_{dt}.pdf',format='pdf', bbox_inches='tight', dpi=300)
