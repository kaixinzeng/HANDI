...
from cyc_compute_nrmse_names import *
...
def main():
    ...
    filename_dt = dt 
    filename_nrmse = [
        args.path_edmd.format(dt=f"{filename_dt:.3f}"),
        args.path_sindy.format(dt=f"{filename_dt:.3f}"), 
        args.path_handi.format(dt=f"{filename_dt:.3f}"),
        args.path_pse.format(dt=f"{filename_dt:.3f}"),
        args.path_gedmd.format(dt=f"{filename_dt:.3f}"),
        args.path_wsindy.format(dt=f"{filename_dt:.3f}"),
        args.path_sr3.format(dt=f"{filename_dt:.3f}")
    ]
    ...
    # method_names_nrmse = ['EDMD', 'SINDy', 'HANDI','PSE','gEDMD']
    # method_names_mse = ['EDMD', 'SINDy', 'HANDI','PSE','gEDMD']
    ...
    try:
        ...
        for idx0, file in enumerate(filename_nrmse):
            ...
            # if method_name == 'PSE':
            #     new_pse_file = file.replace('.txt', '.txt')
    ...
    for n, x0 in enumerate(tqdm(initial_conditions)):
        ...
        # true_is_invariant = check_fixed_structure_invariance(true_solution, eps=args.eps, ratio_thresh=args.ratio_thresh)
...
def parse_arguments():
    ...
    parser.add_argument("--true_equation", type=str, default="../data/cyc_true_equation.txt",help="path to true system equation file")
    parser.add_argument("--path_edmd", type=str, default="../data/cyc_EDMD_dt{dt}_equations.txt")
    parser.add_argument("--path_sindy", type=str,default="../data/cyc_SINDy_dt{dt}_equations.txt")
    parser.add_argument("--path_handi", type=str,default="../results/cyc_HANDI_best_equations_dt{dt}.txt")
    parser.add_argument("--path_pse", type=str,default="../data/cyc_PSE_dt{dt}_equations.txt")
    parser.add_argument("--path_gedmd", type=str,default="../data/cyc_gEDMD_dt{dt}_best_equations.txt")
    parser.add_argument("--path_wsindy", type=str,default="../data/cyc_WSINDy_dt{dt}_equations.txt")
    parser.add_argument("--path_sr3", type=str,default="../data/cyc_SR3_dt{dt}_equations.txt")

    parser.add_argument('--dt', type=float, default=1.0, help='Time step for downsampling/evaluation (dt).')
    parser.add_argument('--t_plot', type=int, default=5, help='Time factor for total integration time (T_max = t_plot * scale).')
    parser.add_argument('--tail_ratio', type=float, default=0.5, help='Trajectory tail ratio (0-1)')
    parser.add_argument('--eps_cyc', type=float, default=0.05, help='Relative tolerance for limit cycle amplitude validation')
    ...
    parser.add_argument('--output_dir', type=str, default='../results', help='Base directory for saving output CSV files.')
