...
system = 'cyc'
file_path = f'../data/{system}_nrmse.xlsx'
...
ylim = ax.get_ylim()
x_min=0.05
y_min=-0.15   # fixed -0.25、 duffing -0.015 、vdp -0.03 、cyc -0.15
ax.set_xlim(left=x_min)  
ax.set_xlim(right=xlim[1]+0.02)
ax.set_ylim(bottom=y_min)
ax.set_ylim(top=ylim[1]+0.10)  # fixed 0.2、duffing 0.015 、vdp 0.02 、cyc 0.10
...
ax.annotate('', xy=(xlim[1]+0.02, y_min), xytext=(x_min, y_min),
            arrowprops=dict(arrowstyle='->', color='black', lw=1.5, mutation_scale=20,shrinkA=0,
                shrinkB=0),
            clip_on=False)
ax.annotate('', xy=(x_min, ylim[1]+0.10), xytext=(x_min, y_min),  # fixed 0.1、duffing 0.015 、vdp 0.01 、cyc 0.05
            arrowprops=dict(arrowstyle='->', color='black', lw=1.5, mutation_scale=20,shrinkA=0,
                shrinkB=0),
            clip_on=False)
...
plt.savefig(f'../results/figure2_{system}_nrmse.pdf', format='pdf', bbox_inches='tight', dpi=300)