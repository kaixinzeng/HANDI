...
methods = [
    {"name": "PSE", "file": f"../data/cyc_PSE_dt{dt:.3f}_equations.txt", "color": "#D8A0A7", 'linewidth':6},
    {"name": "SR3", "file": f"../data/cyc_SR3_dt{dt:.3f}_equations.txt", "color": "#C9A1CB", 'linewidth':6},
    {"name": "SINDy", "file": f"../data/cyc_SINDy_dt{dt:.3f}_equations.txt", "color": "#d06569", 'linewidth':6},
    {"name": "HANDI", "file": f"../results/cyc_HANDI_best_equations_dt{dt:.3f}.txt", "color": "#4FA8D5", 'linewidth':8},
]

fig, ax = plt.subplots(figsize=(6, 4))

plt.plot(x1_true[t_start:t_end], x2_true[t_start:t_end], color='#000000', linestyle=(0, (3, 3)), linewidth=6, label='Truth',zorder=5)
plt.scatter(x1_true[t_start:t_end:int(dt*100)], x2_true[t_start:t_end:int(dt*100)],
           facecolors='#ffffff',edgecolors='#000000', s=100, linewidths=2.5, marker='o', zorder=5)

for m in methods:
    try:
        x1_m, x2_m = load_and_simulate(m["file"], x0, t)
        plt.plot(x1_m[t_start:t_end], x2_m[t_start:t_end], '-', linewidth=m['linewidth'], color=m["color"], label=m["name"], zorder=3)
...
# plt.xlim(-1.3, 2.1)
# ax.set_xticks([-1.2,-0.5,0.2,0.9,1.6])
...
plt.tight_layout()
plt.savefig(f'../results/figure2_cyc_phase_dt{dt}_x1x2.pdf', format="pdf", bbox_inches="tight")
...
plt.tight_layout()
plt.savefig(f'../results/figure2_cyc_trajectory_dt{dt}_x1.pdf', format="pdf", bbox_inches="tight")