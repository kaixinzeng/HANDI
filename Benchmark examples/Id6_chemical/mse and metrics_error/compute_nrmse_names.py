...
def rk4_fixed_step(ode_func, t, x0):
    """Fixed-step RK4 integrator using the provided time grid."""
    x0 = np.array(x0, dtype=float).reshape(-1)
    traj = np.zeros((len(t), len(x0)), dtype=float)
    traj[0] = x0
    for i in range(len(t) - 1):
        h = t[i + 1] - t[i]
        ti = t[i]
        xi = traj[i]
        k1 = np.asarray(ode_func(ti, xi))
        k2 = np.asarray(ode_func(ti + 0.5 * h, xi + 0.5 * h * k1))
        k3 = np.asarray(ode_func(ti + 0.5 * h, xi + 0.5 * h * k2))
        k4 = np.asarray(ode_func(ti + h, xi + h * k3))
        traj[i + 1] = xi + (h / 6.0) * (k1 + 2 * k2 + 2 * k3 + k4)
    return traj
...
def compute_mean_mse_no_inf(mse_dict):
    """Return a dict of mean MSE per method, ignoring inf/NaN entries."""
    mean_mse = {}
    for method, values in mse_dict.items():
        finite_vals = [v for v in values if np.isfinite(v)]
        mean_mse[method] = np.nan if len(finite_vals) == 0 else float(np.mean(finite_vals))
    return mean_mse
    ...
    def model(t, x):
        ...
        for i, eq in enumerate(equations):
            try:
                ...
                raise RuntimeError(
                    f"Equation eval failed: eq='{eq}', x={x}, err={e}"
                )
        return dxdt
    return model
