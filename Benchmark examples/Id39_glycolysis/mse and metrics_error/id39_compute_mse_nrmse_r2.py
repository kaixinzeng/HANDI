...
def main():
    ...
    for n, x0 in enumerate(tqdm(initial_conditions)):
        ...
        for idx,(x1_pred,x2_pred) in enumerate(learned_traj):
            ...
            if (not np.all(np.isfinite(x1_pred))) or (not np.all(np.isfinite(x2_pred))):
                mse_all_points[name].append(1e3)  # cap at 1e3
                all_invalid_counts[name] += 1
                continue

            if len(x1_pred) < len(x1_true):
                pad_len = len(x1_true) - len(x1_pred)
                pad_val_x1 = x1_pred[-1]
                x1_pred = np.concatenate([x1_pred, np.full(pad_len, pad_val_x1)])
                pad_val_x2 = x2_pred[-1]
                x2_pred = np.concatenate([x2_pred, np.full(pad_len, pad_val_x2)])
                all_invalid_counts[name] += 1

            x_pred = np.vstack([x1_pred, x2_pred]).T
...
def parse_arguments():
    ...
    parser.add_argument("--system", type=str, default="id39", help="System name for output csv, e.g., NI")
    parser.add_argument("--noise", type=str, default="0%", help="Noise level directory name, e.g., 0%, 0.125%")
    parser.add_argument("--init_conditions_file", type=str, default="test_benchmark/results/true/id39.npy",help="path to true system equation file")
    parser.add_argument("--true_equation", type=str, default="test_benchmark/results/true/ground_truth_equations_id39.txt",help="path to true system equation file")
    parser.add_argument("--path_edmd", type=str, default="test_benchmark/results/{system}/edmd/{noise}/0.3+3/equations.txt")
    parser.add_argument("--path_sindy", type=str,default="test_benchmark/results/{system}/SINDy/{noise}/0.3+3/sindy_equation_best_0.3+3_stride1.txt")
    parser.add_argument("--path_handi", type=str,default="test_benchmark/results/{system}/HANDI/{noise}/best_equations_dt0.300.txt")
    parser.add_argument("--path_pse", type=str,default="test_benchmark/results/{system}/pse/{noise}/0.3+3/final_ode_system_from_psrn.txt")
    parser.add_argument("--path_gedmd", type=str,default="test_benchmark/results/{system}/gedmd/{noise}/0.3+3/equations.txt")
    parser.add_argument("--path_wsindy", type=str,default="test_benchmark/results/{system}/wsindy/{noise}/equations.txt")
    parser.add_argument("--path_sr3", type=str,default="test_benchmark/results/{system}/SR3/{noise}/tune_best_equations.txt")
    parser.add_argument('--dt', type=float, default=0.3, help='Time step for downsampling/evaluation (dt).')
    parser.add_argument('--t_plot', type=int, default=10, help='Time factor for total integration time (T_max = t_plot * scale).')
    ...
    parser.add_argument('--x_range_min', type=float, default=-0.5, help='Minimum value for initial condition range.')
    parser.add_argument('--x_range_max', type=float, default=0.5, help='Maximum value for initial condition range.')
    parser.add_argument('--y_range_min', type=float, default=-0.5, help='Minimum value for initial condition range.')
    parser.add_argument('--y_range_max', type=float, default=1.0, help='Maximum value for initial condition range.')
    ...
    parser.add_argument('--include_bias', type=bool, default=True, help='Whether to include bias in the feature library.')

    parser.add_argument('--output_dir', type=str, default='test_benchmark/test/id39', help='Base directory for saving output CSV files.')
