...
def main():
    ...
    for n, x0 in enumerate(tqdm(initial_conditions)):
        ...
        print("DEBUG true RHS:", true_model(0, x0))
        ...
        for idx, file in enumerate(filename_mse):
            ...
            ax.plot(x1_true, x2_true, color="#403E3E", linewidth=2, label='True',zorder=2)
...
def parse_arguments():
    ...
    parser.add_argument("--system", type=str, default="id31", help="System name for output csv, e.g., NI")
    parser.add_argument("--noise", type=str, default="0%", help="Noise level directory name, e.g., 0%, 0.125%")
    parser.add_argument("--init_conditions_file", type=str, default="test_benchmark/results/true/id31.npy",help="path to true system equation file")
    parser.add_argument("--true_equation", type=str, default="test_benchmark/results/true/ground_truth_equations_id31.txt",help="path to true system equation file")
    parser.add_argument("--path_edmd", type=str, default="test_benchmark/results/{system}/edmd/{noise}/0.3+2/equations.txt")
    parser.add_argument("--path_sindy", type=str,default="test_benchmark/results/{system}/SINDy/{noise}/0.3+2/sindy_equation_best_0.3+2_stride1.txt")
    parser.add_argument("--path_handi", type=str,default="test_benchmark/results/{system}/HANDI/{noise}/best_equations_dt0.300.txt")
    parser.add_argument("--path_pse", type=str,default="test_benchmark/results/{system}/pse/{noise}/0.3+2/final_ode_system_from_psrn.txt")
    parser.add_argument("--path_gedmd", type=str,default="test_benchmark/results/{system}/gedmd/{noise}/0.3+2/equations.txt")
    ...
    parser.add_argument('--x_range_min', type=float, default=0, help='Minimum value for initial condition range.')
    parser.add_argument('--x_range_max', type=float, default=21.6, help='Maximum value for initial condition range.')
    parser.add_argument('--y_range_min', type=float, default=0, help='Minimum value for initial condition range.')
    parser.add_argument('--y_range_max', type=float, default=31.4, help='Maximum value for initial condition range.')
    ...
    parser.add_argument('--output_dir', type=str, default='test_benchmark/test/id31', help='Base directory for saving output CSV files.')
