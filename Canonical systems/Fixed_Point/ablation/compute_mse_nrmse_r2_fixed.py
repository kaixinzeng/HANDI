...
def main():
    ...
    # method_names_nrmse = ['HANDI','gEDMD', 'wSINDy', 'SR3']
    # method_names_nrmse = ['HANDI','gEDMD', 'wSINDy', 'SR3']
    ...
    for steps, mse_data in mse_slices_total.items():
        ...
        summary_csv_mse_slice = f'{output_dir}/{system}_ablation_average_mse_dt{output_dt_label}_steps_{steps_label}_xyrange{xy_range_max}_grid{grid_size}.csv'
        ...
        boxplot_data_csv = f'{output_dir}/{system}_ablation_mse_dt{output_dt_label}_steps_{steps_label}_xyrange{xy_range_max}_grid{grid_size}.csv'
    ...
    summary_csv_nrmse = f'{output_dir}/{system}_ablation_nrmse_dt{output_dt_label}_xyrange{xy_range_max}_grid{grid_size}.csv'
    ...
    summary_csv_valid = f'{output_dir}/{system}_ablation_valid_rate_dt{output_dt_label}_xyrange{xy_range_max}_grid{grid_size}.csv'
    ...
    summary_csv_r2 = f'{output_dir}/{system}_ablation_r2_dt{output_dt_label}_xyrange{xy_range_max}_grid{grid_size}.csv'
    ...
    average_csv_nppi = f"{output_dir}/{system}_ablation_average_nppi_dt{output_dt_label}_xyrange{xy_range_max}_grid{grid_size}.csv"
    ...
    csv_nppi = f"{output_dir}/{system}_ablation_nppi_dt{output_dt_label}_xyrange{xy_range_max}_grid{grid_size}.csv"
    ...
    csv_mse_all_points = f"{output_dir}/{system}_ablation_mse_all_dt{output_dt_label}_xyrange{xy_range_max}_grid{grid_size}.csv"
...
def parse_arguments():
    ...
    parser.add_argument('--system', type=str, default='fixed',choices=['fixed', 'linear', 'vdp','cyc','duffing'],help='Select system dynamics for ground truth simulation.')
    parser.add_argument("--true_equation", type=str, default="ablation_study/fixed_point/results/true_equation.txt",help="path to true system equation file")
    parser.add_argument("--path_handi", type=str,default="ablation_study/fixed_point/results/residual/best_equations_dt{dt}.txt")
    parser.add_argument("--path_ablationA", type=str,default="ablation_study/fixed_point/results/ablationA/best_equations_dt{dt}.txt")
    parser.add_argument("--path_ortho", type=str,default="ablation_study/fixed_point/results/ortho/best_equations_dt{dt}.txt")
    ...
    parser.add_argument('--xy_range_min', type=float, default=-1, help='Minimum value for initial condition range.')
    parser.add_argument('--xy_range_max', type=float, default=1, help='Maximum value for initial condition range.')
    ...
    parser.add_argument('--output_dir', type=str, default='ablation_study/mse_results/fixed', help='Base directory for saving output CSV files.')
    parser.add_argument('--mse_slices_steps', nargs='*', default=[5, 10], help="Steps for MSE slicing")
