...
def expand_powers_and_replace_spaces(name):
    def replace_power(match):
        var = match.group(1)  # e.g. 'x1'
        exp = int(match.group(2))  # e.g. '3'
        if exp == 0:
            return "1"
        elif exp == 1:
            return var
...
def duffing(x):
    ...
    alpha = -5
    beta = 1
    return np.stack([x2, -lam * x2 - alpha * x1 - beta * x1**3], axis=1)
...
data_filename_pattern="./data/duff_train{}_Nsim10.npy"
OUTPUT_CSV_BASE = "results/sindy_results/sindy_duffing_continue_poly3"
# --- General experiment parameters ---
DELTA_T_VALUES = [0.1, 0.2, 0.3, 0.4]  # down sampled deltaT
STLSQ_THRESHOLD = 0.05  #
NUM_TEST_PTS = 100 #
test_low, test_high = -4.0, 4.0 #
POLYNOMIAL_DEGREE = 3  #
dt = 0.01 #original deltaT 
dim = 2 # dimension of the system

RESULTS_DIR = 'results/sindy_results/'
if not os.path.exists(RESULTS_DIR):
    os.makedirs(RESULTS_DIR)

TRAJECTORY_DIR = 'results/sindy_results/trajectory/'
if not os.path.exists(TRAJECTORY_DIR):
    os.makedirs(TRAJECTORY_DIR)

X_test = make_uniform_test_set(NUM_TEST_PTS, dim, test_low, test_high)
f_true_test = duffing(X_test)
...
for delta_t in tqdm(DELTA_T_VALUES):
    ...
    try:
        ...
        equation_filename = f"SINDY_equation_poly_{POLYNOMIAL_DEGREE}_dt_{delta_t:.2f}_threshold{STLSQ_THRESHOLD}.txt"
    ...
    if not np.isnan(mse):
        ...
        def true_model_ode(t, y):
            return duffing(np.array(y).reshape(1, -1)).flatten()
        ...
        plot_path = os.path.join(f'{os.path.splitext(OUTPUT_CSV_BASE)[0]}_dt{delta_t:.2f}.png')
