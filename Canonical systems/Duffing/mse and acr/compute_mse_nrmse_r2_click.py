...
def main():
    ...
    filename_nrmse = [
        ...
        args.path_pse.format(dt=f"{filename_dt*100:.0f}"),
...
def parse_arguments():
    ...
    parser.add_argument('--system', type=str, default='duffing',choices=['fixed', 'linear', 'vdp','cyc','duffing'],help='Select system dynamics for ground truth simulation.')
    parser.add_argument("--true_equation", type=str, default="duffing/results/true_equation.txt",help="path to true system equation file")
    parser.add_argument("--path_edmd", type=str, default="duffing/results/EDMD_results/EDMD_equation_poly_10_polysysid_3_dt_{dt}.txt")
    parser.add_argument("--path_sindy", type=str,default="duffing/results/sindy_results/SINDY_equation_poly_3_dt_{dt}.txt")
    parser.add_argument("--path_handi", type=str,default="duffing/results/HANDI_results/edmd_runs_10_withL/best_equations_dt{dt}.txt")
    parser.add_argument("--path_pse", type=str,default="duffing/results/PSE_results/duff_train{dt}/final_ode_system_from_psrn.txt")
    parser.add_argument("--path_gedmd", type=str,default="duffing/results/gedmd/gedmd_dt{dt}/best_equations.txt")
    parser.add_argument("--path_wsindy", type=str,default="duffing/results/WSINDy_results/duffing{dt}/tune_best_equations.txt")
    parser.add_argument("--path_sr3", type=str,default="duffing/results/sindysr3_results/duffing{dt}/tune_best_equations.txt")

    parser.add_argument('--dt', type=float, default=0.1, help='Time step for downsampling/evaluation (dt).')
    parser.add_argument('--ht', type=float, default=0.1, help='Time step for dense integration (ht).')
    parser.add_argument('--t_plot', type=int, default=1, help='Time factor for total integration time (T_max = t_plot * scale).')
    ...
    parser.add_argument('--output_dir', type=str, default='nrmse_mse_r2/duffing/duffing0.1', help='Base directory for saving output CSV files.')
