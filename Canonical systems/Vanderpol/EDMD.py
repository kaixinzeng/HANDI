...
if __name__ == "__main__":
    ...
    try:
        ...
        # dx2/dt = -x1 + mu*x2 - mu*x1^2*x2
        idx_x1_for_dx2 = xbasis_name_example.index('x(1)')
        idx_x2_for_dx2 = xbasis_name_example.index('x(2)')
        idx_x1sqx2_for_dx2 = xbasis_name_example.index('x(1)*x(1)*x(2)')  # x1^2 * x2

        weights_true_example[idx_x1_for_dx2, 1] = -1.0
        weights_true_example[idx_x2_for_dx2, 1] = mu
        weights_true_example[idx_x1sqx2_for_dx2, 1] = -mu

        print(f"[Main Setup] Successfully populated weights_true_example for Van der Pol system (mu={mu}).")

    except ValueError as e:
        print(f"[Main Setup] Error populating weights_true_example: {e}")
        print("[Main Setup] One or more basis functions not found. Did you set polyorder >= 3?")
    ...
    default_params = {
        ...
        'deltaT': 0.01,
        'T_sample': np.array([10, 20, 30, 40, 50]),
        'polyorder': polyorder_example,
        'Nbasis_for_sysid': N_basis_example,
        'save_dir': './edmd_results/vdp', # Updated save directory
        'data_filename_pattern': 'vdp_train{}.npy' # NEW: Data filename pattern
    }
